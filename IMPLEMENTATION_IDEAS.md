...
**Description:** Prevent rapid repeated calls by delaying execution until calls stop.
...
- Handle per-instance vs global debouncing
- Consider action ID for tracking
- Provide cancel mechanism
- Handle edge cases (leading/trailing)

---

### 3. Throttling

**Description:** Limit execution frequency to prevent abuse and control load.
...
action
  .throttle({
    ...
    identifier: (ctx) => ctx.user?.id || ctx.ip, // Per-user or per-IP
  })
  .action(async () => {
...
- Handle distributed throttling (Redis-based)
- Provide meaningful error messages when throttled
- Allow bypass for specific users/roles
